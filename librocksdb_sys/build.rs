...
fn bindgen_rocksdb(file_path: &std::path::Path) {
...
fn main() {
    ...
    // DB-1237: on Linux, intercept cacheline_aligned_alloc at link time.
    //
    // The upstream tikv/rocksdb implementation (port/port_posix.cc) silently
    // returns nullptr when posix_memalign fails, which causes a SIGSEGV via
    // CoreLocalArray::data_ == null.  We fix this by compiling a wrapper
    // (cacheline_alloc_fix.cc) with the cc crate — completely independent of
    // cmake — and wiring it in with GNU ld's --wrap flag at the final binary
    // link step.  This approach is immune to cmake build-cache issues.
    if env::var("CARGO_CFG_TARGET_OS").unwrap() == "linux" {
        build.file("crocksdb/cacheline_alloc_fix.cc");
        println!(
